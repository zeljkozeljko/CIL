...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 14 09:54:26 2019
...
pathname0 = '/media/newhd/shared/Data/neutrondata/Feb2018_IMAT_rods/Tomo/Angle_0.0/'
filename0 = 'IMAT00010699_Tomo_000_SummedImg.fits'

data0 = read_fits(pathname0 + filename0)

pathname10 = '/media/newhd/shared/Data/neutrondata/Feb2018_IMAT_rods/Tomo/Angle_16.2306/'
filename10 = 'IMAT00010704_Tomo_000_SummedImg.fits'

data10 = read_fits(pathname10 + filename10)

# Load a flat field (more are available, should we average over them?)
flat1 = read_fits('/media/newhd/shared/Data/neutrondata/Feb2018_IMAT_rods/RB1820541/flat_60min/IMAT00010689_ RB1820541_000_SummedImg.fits')
...
nonzero = flat1 > 0
data0_rel[nonzero] = 2.0* data0[nonzero] / flat1[nonzero].astype(float)
data10_rel =  numpy.zeros(numpy.shape(flat1), dtype = float)
data10_rel[nonzero] = 2.0*data10[nonzero] / flat1[nonzero]
...
plt.figure()
plt.imshow(0.5*flat1.astype('float'))
plt.colorbar()
plt.show()
